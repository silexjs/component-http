	...
	create: function() {
		for(var i in this.ports) {
			...
			if(options.secure === false) {
				options.server = http.createServer(function(req, res) {
				...
				options.server = https.createServer(certificate, function(req, res) {
					self.applyEvents(req, res, true);
				});
			}
		}
		if(this.debug === true) {
			this.log('Servers created');
		}
		return this;
	},
	listen: function() {
		for(var i in this.ports) {
			var options = this.ports[i];
			options.server.listen(options.port, options.ip);
		}
		if(this.debug === true) {
			this.log('Servers listen');
		}
		return this;
	},
};
